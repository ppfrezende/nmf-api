import { ResourceAlreadyExists } from '@/use-cases/_errors/resource-already-exists';
import { ResourceNotFoundError } from '@/use-cases/_errors/resource-not-found-error';
import { makeUpdateTopicUseCase } from '@/use-cases/_factories/project-factories/discipline-factories/topics/make-update-topic-use-case';
import { makeGetProjectsByUserUseCase } from '@/use-cases/_factories/project-factories/make-get-project-by-user-use-case';
import { makeGetUserUseCase } from '@/use-cases/_factories/users-factories/make-get-user-use-case';
import { FastifyRequest, FastifyReply } from 'fastify';
import { z } from 'zod';

export async function updateTopic(
  request: FastifyRequest,
  reply: FastifyReply,
) {
  const updateTopicQuerySchema = z.object({
    projectId: z.string(),
    disciplineId: z.string(),
    topicId: z.string(),
  });
  const updateTopicBodySchema = z.object({
    title: z.string(),
    notes: z.string().optional(),
  });
  const { projectId, disciplineId, topicId } = updateTopicQuerySchema.parse(
    request.params,
  );
  const { title, notes } = updateTopicBodySchema.parse(request.body);

  try {
    const getUserUseCase = makeGetUserUseCase();

    const { user } = await getUserUseCase.execute({
      userId: request.user.sub,
    });

    const getProjectsByUserUseCase = makeGetProjectsByUserUseCase();

    await getProjectsByUserUseCase.execute({
      projectId,
      userId: user.id,
    });

    const updateTopicUseCase = makeUpdateTopicUseCase();

    await updateTopicUseCase.execute({
      disciplineId,
      topicId,
      data: {
        title,
        notes,
      },
    });

    return reply.status(201).send({ message: 'successfully updated' });
  } catch (err) {
    if (err instanceof ResourceAlreadyExists) {
      return reply.status(409).send({ message: err.message });
    }

    if (err instanceof ResourceNotFoundError) {
      return reply.status(404).send({ message: err.message });
    }

    throw err;
  }
}
